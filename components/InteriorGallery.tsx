...
export default function ProjectPage() {
  ...
  const playAmbient = () => {
    if (!ambientRef.current) {
      ambientRef.current = new Audio("https://cdn.pixabay.com/audio/2023/02/28/audio_57d3797fa9.mp3");
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("toggleVoice", onToggle as EventListener);
  }, []);
  ...
  const startAutoTour = async () => {
    ...
    if (speakRef.current) {
      speak("That concludes the guided tour. Thank you for visiting Lumé Interiors.");
      setCurrentProjectIndex(null);
    }
  };
  ...
  const stopNarration = () => {
    ...
    window.dispatchEvent(new CustomEvent("toggleVoice", { detail: { enabled: false } }));
  };
